...
# Configure logging for monitoring database operations
...
def setup_db():
    ...
    conn.commit()
    # Monitor DNS resolution status by logging attempts
    while True:
        try:
            import socket
            ...
            logging.info(f"DNS resolution for {hostname} successful at {time.time()}")
        except Exception as e:
            logging.error(f"DNS resolution for {hostname} failed: {str(e)}")
